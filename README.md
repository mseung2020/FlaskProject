...
### 2️⃣ Market Sentiment — 시장 심리 분석

**"지금 이 종목, 시장의 분위기는 어떤가요?"**

뉴스나 증권사 리포트가 말해주지 않는 **진짜 시장 분위기**를 파악합니다. 종목 토론방에 올라오는 댓글과 뉴스 헤드라인을 분석하여, 개인 투자자들의 체감 심리를 **공포 → 환희** 스펙트럼으로 수치화합니다.

#### 분석 항목

| 분석 대상 | 설명 |
|----------|------|
| **토론방 심리** | 최근 7일간 종목 토론방 댓글을 수집하여 긍정/부정/중립 비율 산출. 좋아요·싫어요 반응을 가중치로 반영하여 더 정확한 여론 파악 |
...
기술적 분석의 핵심인 캔들스틱 패턴을 **자동으로 탐지**해주는 기능입니다. KOSPI 시가총액 상위 50개 종목을 대상으로, 총 20가지 검증된 캔들 패턴을 실시간으로 스캔합니다.
...
- **추세 문맥 반영**: 단순 패턴 형태뿐 아니라 20일 이동평균선 기반 추세 방향도 함께 고려

---

### 4️⃣ Smart Dashboard — 통합 대시보드

**"종목 분석에 필요한 모든 정보를, 한 화면에서."**

기존 HTS(홈트레이딩시스템)처럼 단순히 정보를 나열하는 것이 아니라, 투자 판단에 실제로 필요한 데이터만 **직관적으로 시각화**합니다.

#### 대시보드 구성

| 섹션 | 제공 정보 |
|-----|----------|
| **가격 차트** | 일봉 캔들차트 + 거래량 바차트. 최대 1년치 데이터 조회 가능 |
| **기술적 지표** | MA(5/20/60/120일), MACD, RSI, Stochastic, Williams %R, CCI, ATR, ROC, ADX, 볼린저밴드, Envelope, 일목균형표, Parabolic SAR 등 16개 보조지표 On/Off 선택 |
...
| **심리 점수** | 4가지 축(모멘텀, 시장 폭, 변동성, 주식·채권 선호도)을 종합한 탐욕·공포 지수. 0~100점 스케일로 현재 시장 심리 위치 표시 |
...
| **Breadth** | 상승일 거래량 vs 하락일 거래량 비율 | 탐욕(매수세 강함) |
...
| **패턴 분석** | 차트를 직접 눈으로 확인 | 알고리즘이 자동 탐지 후 알림 |
...
- ✅ *"내가 투자하려는 종목, 심리 지수가 극단적 공포 구간인가?"*
- ✅ *"이 종목에서 상승 반전 패턴이 발생했는가?"*  
